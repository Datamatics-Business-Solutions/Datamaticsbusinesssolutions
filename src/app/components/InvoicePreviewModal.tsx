import { X, Download, CreditCard, FileText, Calendar, Building2 } from 'lucide-react';
...
export function InvoicePreviewModal({ isOpen, onClose, invoice }: InvoicePreviewModalProps) {
  ...
  return (
    <>
      ...
      />
      <div 
        className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl border z-50 animate-scaleIn ${
      ...
      >
        ...
        <div className={`flex items-center justify-between p-6 border-b ${
          ...
          <div className="flex items-center gap-3">
            <FileText className={`w-6 h-6 ${isDark ? 'text-[#E63946]' : 'text-[#BA2027]'}`} />
            <div>
        ...
        <div className="p-8">
            ...
            <table className="w-full">
              <thead className={`${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
                <tr>
                  ...
                  <th className={`px-4 py-3 text-right text-xs font-semibold ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                    Quantity
                  </th>
                  <th className={`px-4 py-3 text-right text-xs font-semibold ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                    Rate
              ...
              <tbody className={`divide-y ${isDark ? 'divide-white/10' : 'divide-gray-200'}`}>
                <tr>
                  ...
                  <td className={`px-4 py-4 text-right ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                    320
                  </td>
                  <td className={`px-4 py-4 text-right ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                    ${(invoice.amount / 320).toFixed(2)}
                  </td>
          ...
          <div className="flex justify-end">
            <div className="w-64 space-y-2">
              ...
              <div className="flex justify-between">
                <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Tax (0%):</span>
                <span className={`text-sm font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>$0</span>
              </div>
              <div className={`flex justify-between pt-2 border-t ${isDark ? 'border-white/10' : 'border-gray-200'}`}>
                ...
                <span className={`text-lg font-bold ${isDark ? 'text-[#E63946]' : 'text-[#BA2027]'}`}>
                  ${invoice.amount.toLocaleString()}
                </span>
              </div>
            </div>
